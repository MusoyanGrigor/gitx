...
impl GraphRenderer {
    ...
    pub fn render(&mut self, commits: &[CommitInfo]) {
        ...
        for (idx, commit) in commits.iter().enumerate() {
            ...
            // 3. Update lanes for children: remove current, add parents
            self.update_lanes(commit, node_lane_idx);
            
            // 4. Render a compact connector row if not the last commit
            if idx < commits.len() - 1 {
                self.render_connector_row();
            }
        }
        ...
        for commit in commits {
            ...
            for p in &commit.parents {
                if visible.contains(p) && !sim_lanes.iter().any(|l| l.as_ref() == Some(p)) {
                     ...
                     if p_pos == sim_lanes.len() { sim_lanes.push(Some(p.clone())); } 
                     else { sim_lanes[p_pos] = Some(p.clone()); }
                }
            }
    ...
    fn get_or_assign_lane(&mut self, hash: &str) -> usize {
        if let Some(pos) = self.active_lanes.iter().position(|l| l.as_ref() == Some(&hash.to_string())) {
            ...
            if pos == self.active_lanes.len() { self.active_lanes.push(Some(hash.to_string())); } 
            else { self.active_lanes[pos] = Some(hash.to_string()); }
            pos
        }
    }
    ...
    fn update_lanes(&mut self, commit: &CommitInfo, node_lane_idx: usize) {
        ...
        for p in &commit.parents {
            if self.visible_hashes.contains(p) && !self.active_lanes.iter().any(|l| l.as_ref() == Some(p)) {
                let pos = self.active_lanes.iter().position(|l| l.is_none()).unwrap_or(self.active_lanes.len());
                if pos == self.active_lanes.len() { self.active_lanes.push(Some(p.clone())); } 
                else { self.active_lanes[pos] = Some(p.clone()); }
            }
        }
    ...
    fn render_node_row(&self, commit: &CommitInfo, node_lane_idx: usize, graph_width: usize) {
        ...
        let node_style = if is_head { TreeStyle::head_node() } 
                         else if is_merge { TreeStyle::merge_node(node_lane_idx) }
                         else { TreeStyle::commit_node(node_lane_idx) };
        ...
        for (i, lane) in self.active_lanes.iter().enumerate() {
            if i == node_lane_idx {
                ...
                graph.push_str(&styled("│", TreeStyle::connector(i)));
            } else {
                graph.push(' ');
            }
            graph.push(' ');
        ...
        // Fill up to graph_width for alignment
        let current_width = self.active_lanes.len() * 2;
        if current_width < graph_width {
            for _ in 0..(graph_width - current_width) { graph.push(' '); }
        }
