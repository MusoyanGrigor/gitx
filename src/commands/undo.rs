...
use crate::models::undo::{ResetMode, UndoAction, UndoPlan};
    ...
    match sub {
        ...
        UndoSubcommand::Unstage => {
            plan.actions = plan
                .staged_files
                .iter()
                .map(|f| UndoAction::Unstage(f.clone()))
                .collect();
        }
        UndoSubcommand::Discard => {
            plan.actions = plan
                .unstaged_files
                .iter()
                .map(|f| UndoAction::Discard(f.clone()))
                .collect();
        }
        UndoSubcommand::LastCommit {
            soft,
            mixed: _,
            hard,
        } => {
            if let Some(ref info) = plan.last_commit {
                ...
                plan.actions.push(UndoAction::ResetCommit {
        ...
        UndoSubcommand::Clean { .. } => {
            plan.actions = plan
                .untracked_files
                .iter()
                .map(|f| UndoAction::RemoveUntracked(f.clone()))
                .collect();
        }
        UndoSubcommand::All { .. } => {
            for f in &plan.staged_files {
                plan.actions.push(UndoAction::Unstage(f.clone()));
            }
            for f in &plan.unstaged_files {
                plan.actions.push(UndoAction::Discard(f.clone()));
            }
            for f in &plan.untracked_files {
                plan.actions.push(UndoAction::RemoveUntracked(f.clone()));
            }
        }
    }
...
fn print_undo_status(plan: &UndoPlan) {
    ...
    if let Some(ref info) = plan.last_commit {
        ...
        let hash_short = if info.hash.len() > 7 {
            &info.hash[..7]
        } else {
            &info.hash
        };
        ...
        if info.already_pushed {
            println!(
                "  {}",
                "WARNING: This commit appears to be already pushed to a remote."
                    .red()
                    .italic()
            );
        }
